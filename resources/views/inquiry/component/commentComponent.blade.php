    ...
    (function () {
        ...
        updateTextArea = "<div class='row'>"
            +"<div class='col' id='comments{{$comment->id}}'>"
                +"<form class='form-group' action='{{route('inquiry.comment.update',['boardNum'=>$inquiry->num,'search'=>$search,'where'=>$where,'page'=>$page,'commentId'=>$comment->id])}}' method='post'>"
                    +'@csrf'
                    +'@method("put")'
                    ...
                    +"<div class='row'>"
                        ...
                        + "<div class='col-2'><input type='submit' class='btn btn-outline-danger' value='@lang('home/component/commentComponent.modify')' id='commentWrite'>"
                            +"<i id='closeBtn{{$comment->id}}' title='close' class='float-right pnt'>x</i>"
                        +"</div>"
                +"</form>"
            +"</div>"
        +"</div>";
